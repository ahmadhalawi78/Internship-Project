                        ...
                        {listing.listing_images && listing.listing_images.length > 1 && (
                            <div className="flex gap-2 overflow-x-auto pb-2">
                                {listing.listing_images.slice(1).map((img: any) => (
                                    <div key={img.id} className="relative h-20 w-20 flex-shrink-0 overflow-hidden rounded-lg border border-slate-200">
                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-4 text-slate-500 mt-2">
                                <div className="flex items-center gap-1.5">
                                    ...
                                    <span className="text-sm font-medium">{listing.location || "Location not specified"}</span>
                                </div>
                                <div className="flex items-center gap-1.5">
                        ...
                        <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="flex flex-col">
                                    ...
                                    <span className="font-semibold text-slate-700 capitalize">{listing.type}</span>
                                </div>
                                {listing.quantity && (
                                    <div className="flex flex-col">
                                        <span className="text-xs text-slate-400 font-bold uppercase">Quantity</span>
                                        <span className="font-semibold text-slate-700">{listing.quantity}</span>
                                    </div>
                                )}
                                {listing.status && (
                                    <div className="flex flex-col">
                                        <span className="text-xs text-slate-400 font-bold uppercase">Status</span>
                                        <span className={`font-semibold capitalize ${listing.status === 'available' ? 'text-emerald-600' : 'text-slate-700'}`}>
                                            {listing.status}
                                        </span>
                                    </div>
                                )}
